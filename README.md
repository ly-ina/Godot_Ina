...
`godot-mcp-server` 是一个基于 Model Context Protocol (MCP) 的服务器，让 AI 助手可以直接读写 Godot 项目文件。**完成了全部 4 个开发阶段，271 个测试用例，20 个 MCP Tools。** 它是连接 AI 与 Godot 的桥梁，使 AI 能够：

- 读取和解析 `.tscn` 场景文件
- 创建和修改 Godot 场景
- 管理 GDScript 文件（创建、读写、搜索替换、翻译）
- 通过 Godot CLI 运行项目和执行代码
- 校验场景和项目结构
- 跨场景搜索引用

...
#### 阶段 B — 系统化（当前）
- [ ] `generate_game` 按类型产出完整可运行 .tscn
- [ ] 所有生成系统共用同一色调/尺寸配置
- [ ] 各生成系统可叠加到同一个 Main.tscn（NPC + 装备 + 世界）
- [ ] 首个完整多系统 demo：MC 世界 + 商人 NPC + 背包
- [ ] 插件化：外部开发者可写自己的生成器
...
| 🔴 P0 | `generate_game` 所有 type 产出完整可运行场景 | 进行中 |
| 🟠 P1 | 生成系统共享着色/尺寸配置 | 待开始 |
| 🟠 P1 | 多系统叠加 demo（世界 + NPC + 背包） | 待开始 |
| 🟡 P2 | 插件化：外部开发者接入 | 待开始 |
| 🔵 P3 | 精灵表生成 / 3D / npm 发布 | 远期 |
